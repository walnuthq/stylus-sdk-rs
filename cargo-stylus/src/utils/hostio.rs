...
/// Returns (status, should_use_trace_status) where should_use_trace_status indicates
/// whether we got a valid trace and should use its status
fn handle_solidity_call(
    ...
    let config = config?;
    let contract_info = config.solidity_contracts.get(&addr_str)?;
