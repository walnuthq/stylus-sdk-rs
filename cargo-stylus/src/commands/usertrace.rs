...
/// Format: { "contracts": [{ "address": "0x...", "environment": "evm", "name": "...",
        ...
        if let (Some(new_cid), Some(new_parent)) = (new_call_id, new_call_parent) {
            ...
            for entry in &lldb_trace {
                ...
                let pid = entry
                    ...
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0);
                if pid == new_parent && cid > new_cid && best_sibling.is_none_or(|b| cid < b) {
...
mod tests {
    ...
    fn test_merge_cross_env_traces_propagates_evm_errors() {
        ...
        assert!(evm_call.get("error").unwrap().as_bool().unwrap());
        assert_eq!(
