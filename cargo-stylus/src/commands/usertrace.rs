...
impl From<ContractConfig> for soldb_bridge::SolidityContractInfo {
    fn from(config: ContractConfig) -> Self {
        soldb_bridge::SolidityContractInfo {
            name: config.name,
            debug_dir: config.debug_dir,
            project_path: Some(config.project_path),
        }
    }
}
...
fn init_cross_env_bridge(
    ...
    hostio::set_cross_env_config(soldb_bridge::CrossEnvConfig {
        ...
        solidity_contracts: solidity_contracts
            .iter()
...
/// Merge cross-env (Solidity) traces into the LLDB function trace JSON.
///
/// This reads `/tmp/cross_env_traces.json` (written by hostio.rs child process)
/// and merges the Solidity calls as children of the matching Stylus calls in
/// `/tmp/lldb_function_trace.json` (written by LLDB calltrace).
fn merge_cross_env_traces(lldb_trace_path: &str) -> eyre::Result<bool> {
    let cross_env_path = hostio::CROSS_ENV_TRACES_PATH;
...
async fn exec_inner(args: Args) -> eyre::Result<()> {
    ...
    if !args.child {
        ...
        if let Err(e) = merge_cross_env_traces("/tmp/lldb_function_trace.json") {
