...
We also support transaction simulation via the `usertrace` command with `--simulate` flag. This effectively runs a `usertrace` without requiring a transaction hash on-chain:

```bash
cargo stylus-beta usertrace --simulate \
  --from 0x3f1Eae7D46d88F08fc2F8ed27FCb2AB183EB2d0E \
